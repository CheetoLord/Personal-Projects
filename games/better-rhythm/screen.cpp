...
void screen::drawText(string text, int x, int y) {
    ...
    for(int i = 0; i < text.length(); i++) {
        ...
        if(currChar == '\n') {
            currY += myFontSize + (myFontSize/FONTDETAIL);
            currX = x;
            continue;
        } else if(currChar == ' ') {
...
int screen::getTextHeight(string text) {
    ...
    int spacing = myFontSize/FONTDETAIL;
    int height = spacing*(newLineCount) + myFontSize*(newLineCount+1);
    return height;
}
...
void inputBox::draw() {
    ...
    if(myText.length() > 0 || isSelected) {
        ...
        if(isSelected) {
            char addingChar = (cursorFlashTimer % 80) >= 40 ? ' ' : '_';
            displayText += addingChar;
        }
    } else {
