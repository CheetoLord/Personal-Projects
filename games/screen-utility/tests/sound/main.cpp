...
static string device = "default";            /* playback device */
...
int runSounds(int argc, char* argv[])
{
    int err;
    snd_pcm_t *handle;
    ...
    WaveType wTypeL = wave_Saw;
    WaveType wTypeR = wave_Saw;
    double freqL = 440.0, freqR = 880.0;
    double phaseL = 0.0, phaseR = 0.0;
    double gainL = 80.0 * 1E+7, gainR = 80.0 * 1E+7;
...
int main(int argc, char* argv[]) {
    ...
    if(!useClass) {
        ...
        soundHandler myHandler;
        myHandler.initAudioDevice();
        //myHandler.loadSound("sounds/testfile.wav", "test");
        myHandler.loadSound("sounds/Rick-Astley-Never-Gonna-Give-You-Up-Official-Music-Video.wav", "rickroll");
        myHandler.playSound("rickroll");
        while (true) {
            myHandler.runSounds();
        }
         myHandler.sanityCheck();
        // myHandler.playSection(0);
        // myHandler.playSection(1);
        // myHandler.playSection(2);
    }
